...
var recast25 = __toESM(require("recast"));
...
var appendElementToRoot = ({
  ...
  if (argument.type === "JSXFragment") {
    argument.children.push(stripParenthesizedExtra(element));
  } else {
...
var getEnclosingReturnStatement = (path) => {
...
var getCompositionOrFolderLabel = (item) => {
  return item.type === "composition" ? `composition "${item.compositionId}"` : `folder "${[item.parentName, item.folderName].filter(Boolean).join("/")}"`;
};
var matchesCompositionOrFolder = ({
  item,
  node,
  parentFolderName
}) => {
  ...
  return getFolderNameFromJSXElement(node) === item.folderName && parentFolderName === item.parentName;
};
var moveCompositionOrFolder = ({
  file,
  transformation,
  changesMade
}) => {
  ...
  const folders = [];
  const folderStack = [];
  const visitJsxElement = (astPath) => {
    const node = astPath.node;
    const parent = astPath.parentPath?.node;
    const parentFolderName = folderStack.join("/") || null;
    const isDirectJsxChild = (parent?.type === "JSXElement" || parent?.type === "JSXFragment") && parent.children.includes(node);
    if (isDirectJsxChild && matchesCompositionOrFolder({
    ...
    if (isDirectJsxChild && (transformation.destination.type === "before" || transformation.destination.type === "after") && matchesCompositionOrFolder({
    ...
    if (isDirectJsxChild && transformation.destination.type === "folder" && folderName === transformation.destination.folderName && parentFolderName === transformation.destination.parentName) {
      destinationFolder = {
        node,
        parent,
        parentFolderName,
    ...
    for (let index = 0;index < node.children.length; index++) {
      if (node.children[index].type === "JSXElement") {
        visitJsxElement(astPath.get("children", index));
      }
    }
    if (folderName !== null) {
      folderStack.pop();
  ...
  if (source === null) {
    throw new Error(`Could not find ${getCompositionOrFolderLabel(transformation.source)} as a direct JSX child`);
  }
  const sourceItem = source;
  if ((transformation.destination.type === "before" || transformation.destination.type === "after") && target === null) {
    throw new Error(`Could not find ${getCompositionOrFolderLabel(transformation.destination.target)} as a reorder target`);
  }
  if (transformation.destination.type === "folder" && destinationFolder === null) {
    const folderPath = [
  ...
  if (target !== null && target.node === sourceItem.node) {
    return { newAst: file, changesMade };
  }
  const sourceFolderPath = transformation.source.type === "folder" ? [transformation.source.parentName, transformation.source.folderName].filter(Boolean).join("/") : null;
  const destinationParentFolderName = transformation.destination.type === "root" ? null : transformation.destination.type === "folder" ? [
    transformation.destination.parentName,
    transformation.destination.folderName
  ].filter(Boolean).join("/") : target.parentFolderName;
  if (sourceFolderPath !== null && destinationParentFolderName !== null && (destinationParentFolderName === sourceFolderPath || destinationParentFolderName.startsWith(`${sourceFolderPath}/`))) {
    throw new Error("A folder cannot be moved inside itself");
  }
  if (transformation.source.type === "folder") {
    const sourceFolderName = transformation.source.folderName;
    if (folders.some((folder) => folder.node !== sourceItem.node && folder.name === sourceFolderName && folder.parentFolderName === destinationParentFolderName)) {
      throw new Error(`A folder named "${sourceFolderName}" already exists in the destination`);
    }
  }
  ...
  if (transformation.destination.type === "root") {
    ...
    appendElementToFolder({
      element,
      folderElement: destinationFolder.node
    });
  } else {
    ...
    targetItem.parent.children.splice(transformation.destination.type === "before" ? targetIndex : targetIndex + 1, 0, element);
  }
...
var getTopLevelReturnStatement = (statements) => {
  ...
  for (const statement of statements) {
    if (recast19.types.namedTypes.ReturnStatement.check(statement)) {
...
var createSequenceElement = () => {
  return recast19.types.builders.jsxElement(recast19.types.builders.jsxOpeningElement(recast19.types.builders.jsxIdentifier("Sequence"), []), recast19.types.builders.jsxClosingElement(recast19.types.builders.jsxIdentifier("Sequence")), []);
};
var createNumberAttribute = (name, value) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier(name), recast19.types.builders.jsxExpressionContainer(recast19.types.builders.numericLiteral(value)));
};
var createStringAttribute = (name, value) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier(name), recast19.types.builders.stringLiteral(value));
};
var createBooleanAttribute = (name, value) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier(name), recast19.types.builders.jsxExpressionContainer(recast19.types.builders.booleanLiteral(value)));
};
...
var createStyleAttribute = (properties) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier("style"), recast19.types.builders.jsxExpressionContainer(recast19.types.builders.objectExpression(properties)));
};
var getPositionStyleProperties = (position) => {
  const properties = [
    recast19.types.builders.objectProperty(recast19.types.builders.identifier("position"), recast19.types.builders.stringLiteral("absolute"))
  ];
  if (position) {
    properties.push(recast19.types.builders.objectProperty(recast19.types.builders.identifier("translate"), recast19.types.builders.stringLiteral(formatTranslateValue(position))));
  }
  return properties;
};
...
var createAssetStyleAttribute = ({
  ...
  return createStyleAttribute([
    ...
    ...dimensions ? [
      recast19.types.builders.objectProperty(recast19.types.builders.identifier("width"), recast19.types.builders.numericLiteral(dimensions.width)),
      recast19.types.builders.objectProperty(recast19.types.builders.identifier("height"), recast19.types.builders.numericLiteral(dimensions.height))
    ] : []
  ]);
};
var createStaticFileSrcAttribute = ({
  ...
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier("src"), recast19.types.builders.jsxExpressionContainer(recast19.types.builders.callExpression(recast19.types.builders.identifier(staticFileLocalName), [recast19.types.builders.stringLiteral(src)])));
};
...
var createStringSrcAttribute = (src) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier("src"), recast19.types.builders.stringLiteral(src));
};
var createSolidElement = ({
  ...
  return recast19.types.builders.jsxElement(recast19.types.builders.jsxOpeningElement(recast19.types.builders.jsxIdentifier(localName), [
...
var createSequenceWrappedElement = ({
  ...
  return recast19.types.builders.jsxElement(recast19.types.builders.jsxOpeningElement(recast19.types.builders.jsxIdentifier(sequenceLocalName), [
    ...from === null ? [] : [createNumberAttribute("from", from)],
    ...
    createPositionAbsoluteStyleAttribute(position)
  ], false), recast19.types.builders.jsxClosingElement(recast19.types.builders.jsxIdentifier(sequenceLocalName)), [child]);
};
...
var createSvgElement = async ({
  ...
  const interactiveSvgName = () => recast19.types.builders.jsxMemberExpression(recast19.types.builders.jsxIdentifier(interactiveLocalName), recast19.types.builders.jsxIdentifier("Svg"));
...
var createFragmentWithElement = (element) => {
  return recast19.types.builders.jsxFragment(recast19.types.builders.jsxOpeningFragment(), recast19.types.builders.jsxClosingFragment(), [element]);
};
...
var addOfficialNamedImport = ({
  ...
  const importSpecifier = recast19.types.builders.importSpecifier(recast19.types.builders.identifier(importedName));
  if (existingImport) {
    existingImport.specifiers = [
      ...existingImport.specifiers ?? [],
      importSpecifier
  ...
  const importDeclaration = recast19.types.builders.importDeclaration([importSpecifier], recast19.types.builders.stringLiteral(sourcePath));
  insertImportDeclaration(ast, importDeclaration);
};
...
var identifierRegex3 = /^[A-Za-z_$][0-9A-Za-z_$]*$/;
var toPascalCaseIdentifier = (value) => {
  ...
  return identifierRegex3.test(candidate) ? candidate : "CompositionComponent";
};
...
var ensureDefaultImport = ({
  ...
  const importSpecifier = recast19.types.builders.importDefaultSpecifier(recast19.types.builders.identifier(localName));
...
var createExpressionAttribute = (name, value) => {
  return recast19.types.builders.jsxAttribute(recast19.types.builders.jsxIdentifier(name), recast19.types.builders.jsxExpressionContainer(parseValueExpression(value)));
};
...
var createCompositionObjectProperty = ({
  ...
  return recast19.types.builders.objectProperty(identifierRegex3.test(name) ? recast19.types.builders.identifier(name) : recast19.types.builders.stringLiteral(name), parseValueExpression(value));
};
var createCompositionComponentElement = ({
  ...
  const attributes = [
    ...
    ...spreadProperties.length === 0 ? [] : [
      recast19.types.builders.jsxSpreadAttribute(recast19.types.builders.objectExpression(spreadProperties))
    ]
  ];
  return recast19.types.builders.jsxElement(recast19.types.builders.jsxOpeningElement(recast19.types.builders.jsxIdentifier(localName), attributes, true), null, []);
};
...
var addElementToComponentRoot = ({
  ...
  if (rootNode.type === "JSXElement") {
    ...
    const fragment = recast19.types.builders.jsxFragment(recast19.types.builders.jsxOpeningFragment(), recast19.types.builders.jsxClosingFragment(), [existingRoot, element]);
    let replaced = false;
    recast19.types.visit(ast, {
      visitJSXElement(astPath) {
...
var { builders: b11, namedTypes: namedTypes3 } = recast20.types;
...
var cloneJsxElement2 = (element) => {
  const printed = recast20.print(element).code;
  const file = parseAstForReadOnly(`<>${printed}</>;`);
  const statement = file.program.body[0];
  if (statement?.type !== "ExpressionStatement" || statement.expression.type !== "JSXFragment") {
...
var indentContinuationLines = ({
  indent,
  source
}) => {
  const endOfLine = source.includes(`\r
`) ? `\r
` : `
`;
  return source.split(/\r?\n/).join(`${endOfLine}${indent}`);
};
var getSplitSourceEdit = ({
  input,
  left,
  prettierConfigOverride,
  right,
...
`;
  ...
  const print6 = (element) => indentContinuationLines({
    indent: lineIndent,
  ...
  const replacement = wrapInFragment ? print6(makeFragment3(left, right)) : `${print6(left)}${isOnlyElementOnLine ? `${endOfLine}${lineIndent}` : " "}${print6(right)}`;
  return { end, replacement, start };
};
var splitJsxSequence = ({
  input,
  nodePath,
  sequenceKeys,
  splitFrame,
  ...
  const isJsxChild = namedTypes3.JSXElement.check(parentPath.node) || namedTypes3.JSXFragment.check(parentPath.node);
  ...
  const output = input.slice(0, sourceEdit.start) + sourceEdit.replacement + input.slice(sourceEdit.end);
  const { nodePathRemappings } = getNodePathRemappings({
    ast,
    captured: capturedNodePaths,
    output
...
var { namedTypes: namedTypes4 } = recast21.types;
var audioProps = [
  "src",
  "from",
  "durationInFrames",
...
var b12 = recast23.types.builders;
...
var updateSequenceKeyframesAst = ({
  ...
  for (const update of updates) {
    ...
    oldValueStrings.push(recast23.print(prop.expression).code);
    ...
    newValueStrings.push(recast23.print(nextExpression).code);
...
var b13 = recast24.types.builders;
...
var snapshotTopLevelAttrs = (node) => {
  ...
  for (const a2 of node.attributes ?? []) {
    if (a2.type === "JSXAttribute" && a2.name.type === "JSXIdentifier") {
      const { name } = a2.name;
      const printed = recast24.print(a2).code.replace(/\s+/g, " ").replace(/,(\s*[}\]])/g, "$1").trim();
      const prefix = `${name}=`;
...
var isFunctionNode2 = (value) => {
  return recast24.types.namedTypes.FunctionDeclaration.check(value) || recast24.types.namedTypes.FunctionExpression.check(value) || recast24.types.namedTypes.ArrowFunctionExpression.check(value);
};
var ensureUseCurrentFrameHook2 = ({
  ...
  if (!recast24.types.namedTypes.BlockStatement.check(fn.body)) {
  ...
  for (const { key, value, defaultValue, clipboardParam } of updates) {
    ...
    if (isNested) {
      ...
      if (attr && attr.type !== "JSXSpreadAttribute" && attr.value) {
        const printed = recast24.print(attr.value).code;
...
`);
  ...
  const replacements = openingElementLocations ? [...openingElementLocations].map(([openingElement, location]) => {
    ...
    return {
      ...
      replacement: recast24.print(openingElement).code,
...
var getNodePathRemappings2 = ({
  ...
  const before = [];
  const after = [];
  recast25.visit(astBefore, {
    visitJSXOpeningElement(path) {
      before.push({
        nodePath: getNodePathForRecastPath(path, astBefore),
        signature: recast25.print(path.node).code
      });
      return this.traverse(path);
    }
  });
  recast25.visit(astAfter, {
    visitJSXOpeningElement(path) {
      after.push({
